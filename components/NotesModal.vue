            ...
            <template v-for="n in notes" :key="n.id">
              <li class="flex items-start justify-between rounded-lg border p-5">
                ...
                <div class="flex shrink-0 items-center gap-2">
                  <button @click="setValues(n)" class="btn-icon p-1.5" type="button">
                    <Icon size="20" name="solar:pen-new-square-line-duotone" />
                  </button>

                  <!-- Delete button -->
                  <VDropdown>
                    <button class="btn-icon p-1.5" type="button">
...
<script setup lang="ts">
  ...
  import type { Contact, Notes } from '@prisma/client';
  ...
  const deleteNote = async (id: string | number, hide: () => void) => {
