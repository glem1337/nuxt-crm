<template>
  <main class="flex h-[100dvh] items-center justify-center">
    <div class="w-screen p-5 md:w-[400px]">
      <h1 class="text-2xl font-bold lg:text-4xl">Login</h1>
      ...
      <form @submit="onSubmit" class="mt-10">
        ...
        <div class="mt-5">
          <button :disabled="isSubmitting" class="btn btn-primary w-full uppercase">Sign in</button>

          <p class="mt-7 text-sm">
            <NuxtLink to="/register">
              Don't have an account?
              <span class="font-medium text-primary">Create one</span>
            </NuxtLink>
          </p>
        </div>
      </form>
...
<script lang="ts" setup>
  ...
  const onSubmit = handleSubmit(async (values) => {
    try {
      const { login } = useAuth();
      await useToast().promise(login(values), {
